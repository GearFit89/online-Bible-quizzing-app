...
class Sockets {
    ...
    handleConnection(onClose, onError) {
        this.clientWs.onclose = (event) => { // COMMENT: Defines the handler for connection closure.
            ...
            if (typeof onClose === 'function') onClose(this.clientWs); // COMMENT: Correctly calls the external onClose callback if provided.
        };
        this.clientWs.onerror = (error) => { // COMMENT: Defines the handler for connection errors.
            console.error('Error', error);
            if (typeof onError === 'function') onError(this.clientWs, error); // COMMENT: Correctly calls the external onError callback if provided.
        };
